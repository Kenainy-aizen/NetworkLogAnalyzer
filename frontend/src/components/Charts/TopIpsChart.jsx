...
export default function TopIpsChart({ events, onIpClick }) {
    ...
    <ResponsiveContainer width="100%" height={200}>
      <BarChart data={data} layout="vertical" margin={{ top: 8, right: 16, left: 8, bottom: 0 }}>
        ...
        <YAxis
          ...
          fontSize={11}
          width={110}
          tick={{ cursor: 'pointer', fill: '#60a5fa' }}
          onClick={(data) => onIpClick?.(data.value)}
        />
        <Tooltip
        ...
        <Bar
          ...
          radius={[0, 4, 4, 0]}
          cursor="pointer"
          onClick={(data) => onIpClick?.(data.ip)}
        >
          {data.map((entry) => (
            <Cell key={entry.ip} fill="#3b82f6" />
          ))}
        </Bar>
      </BarChart>
    </ResponsiveContainer>
